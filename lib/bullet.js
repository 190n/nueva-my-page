...
const BulletProto = _.merge({}, BaseEntity, Movement, {
    ...
    collideAll() {
        ...
        for (let b of this.manager.getEntitiesByType('bomb')) {
            if (util.boundingBoxCollide(this, b)) {
                this.manager.removeEntities(this);
                b.manager.removeEntities(b);
                return;
            }
        }

        let im = this.manager.getEntitiesByType('invader manager')[0];
        for (let c of im.entities) {
            for (let i of c.entities) {
