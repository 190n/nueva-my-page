...
const InvaderManagerProto = _.merge({}, BaseEntity, CompoundEntity, {
    ...
    updateAll(dt) {
        ...
        if (!e) return this.turn = 0;
        e.update(dt);
        if (util.boundingBoxCollide(e, {x: -MARGIN, y: 0, width: MARGIN, height: HEIGHT})
            || util.boundingBoxCollide(e, {x: WIDTH, y: 0, width: MARGIN, height: HEIGHT})) {
            this.newDx = -e.dx;
