...
async function main() {
    blinkCursor();
    let intro = localStorage.skipping == 'true' ? SKIP_INTRO : INTRO;

    if (localStorage.skipping == 'true') {
        localStorage.skipping = 'false';
    }
    for (let c of intro) {
        if (typeof c == 'number') {
            await wait(c);
            ...
            switch (c) {
                case '!!wait-for-click!!':
                    if (localStorage.seenWaitMessage == 'true') {
                        document.getElementById('maximize-message').style.display = 'none';
                        break;
                    }

                    localStorage.seenWaitMessage = 'true';
                    let waiting = true;
                    document.getElementById('maximize-message').onclick = function() {
                        waiting = false;
                        document.getElementById('maximize-message').style.display = 'none';
                    }
                    while (waiting) {
                ...
                case '!!write!!':
                    ...
                    for (let c of chars) {
                        await wait(Math.round(Math.random() * 75) + 50);
                        page.textContent += c;
                    }
                    break;
